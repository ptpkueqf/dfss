  		...
  		for(;;)
		{
			...
			if(pID == 0) //child process:
			{
				...
				do
				{
					...
					//printf("[FTP-SERVER] serving message of type: %s\n", msgBuffer);
								...
								if(sizeof(msgBuffer) == 0)//if buffer is empty
								{
									if(strcmp(directory->d_name, ".") == 0 ||
									   strcmp(directory->d_name, "..") == 0)
										...
										printf("");//catch unnecessary info
									else
									{
										//printf("\n%s", directory->d_name);
										sprintf(msgBuffer, "%s;", directory->d_name);
									}
								}
						...
						msgBuffer[3] == ' ')
					{
						...
						printf("[FTP-SERVER]File: '%s' (%d bytes)\n", fileName, fileSize);
